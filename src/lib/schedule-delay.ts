...
export function parseDelayMs(str: string): number | null {
	const trimmed = (str || '').trim().toLowerCase();
	if (!trimmed) return null;
	const m = trimmed.match(/^(?:(\d+)d)?(?:(\d+)h)?(?:(\d+)m)?(?:(\d+)s)?$/);
	if (!m || (!m[1] && !m[2] && !m[3] && !m[4])) return null;
	const ms = (
		(parseInt(m[1] || '0', 10) * 86_400) +
...
export function isValidDelayMs(ms: number): boolean {
	return Number.isFinite(ms) && ms >= 1 && ms <= MAX_SCHEDULE_MS;
}

export function getScheduleDelayError(body: unknown): string | null {
	if (!body || typeof body !== 'object') return null;
	const delayMs = (body as Record<string, unknown>).delayMs;
	if (typeof delayMs !== 'number' || !Number.isFinite(delayMs)) return null;
	if (delayMs < 1 || delayMs > MAX_SCHEDULE_MS) return DELAY_RANGE_MESSAGE;
	return null;
}

export function scheduleDelayParseScript(): string {
	return `
