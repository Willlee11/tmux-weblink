...
export interface PaneInfo {
	paneId: string;      // stable id, e.g. "%5"
	paneIndex: number;
	windowIndex: number;
	pid: number;         // pane's foreground-process pid (tmux #{pane_pid})
	command: string;     // tmux #{pane_current_command}
	active: boolean;     // active pane within its window
	windowActive: boolean; // belongs to the session's active window
	/** Target usable with capture-pane, e.g. "session:0.1". */
	target: string;
}

const PANE_FORMAT =
	"#{pane_id}\t#{pane_index}\t#{window_index}\t#{pane_pid}\t#{pane_current_command}\t#{pane_active}\t#{window_active}";

function parsePaneLine(session: string, line: string): PaneInfo | null {
...
export function getActivePaneInfo(session: string): PaneInfo | null {
