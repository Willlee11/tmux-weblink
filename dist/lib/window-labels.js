...
export function listWindowLabels(sessionName) {
    return (db.data.windowLabels ?? []).filter((record) => record.sessionName === sessionName);
}
export async function setWindowLabel(sessionName, windowIndex, label) {
    const labels = db.data.windowLabels ??= [];
    const trimmed = label.trim();
    const idx = labels.findIndex((record) => record.sessionName === sessionName && record.windowIndex === windowIndex);
    if (!trimmed) {
