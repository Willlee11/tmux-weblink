...
export async function cmdAdd(pkg) {
...
export async function cmdRemove(pkg) {
...
async function runNpmAsync(args) {
    try {
...
export async function installPlugin(pkg) {
    if (!isValidPackageName(pkg))
        return { ok: false, output: `invalid package name: ${pkg}` };
    await ensureInstallDir();
...
export async function uninstallPlugin(pkg) {
    if (!isValidPackageName(pkg))
        return { ok: false, output: `invalid package name: ${pkg}` };
    let output = '';
    if (existsSync(path.join(PLUGIN_DIR, 'node_modules', pkg))) {
