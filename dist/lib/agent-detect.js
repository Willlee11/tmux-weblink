...
function agentFromBasename(name) {
    switch (name) {
        case 'claude':
        case 'claude-code':
            return 'claude';
 ...
 */
export function identifyAgent(command, argvTexts = []) {
    ...
    if (!isRuntime && argvTexts.length === 0)
        return null;
...
function hasInterruptHint(liveLower) {
    return (liveLower.includes('esc to interrupt') ||
 ...
 */
function hasActiveSpinnerLine(lines) {
    return lines.some((l) => /^[·✢✳✶✻✽*] \S.*…/.test(l));
}
function detectClaude(lines, liveLower) {
    ...
    if (hasInterruptHint(liveLower) || hasActiveSpinnerLine(lines))
        return 'working';
    return 'idle';
}
function detectCodex(lines, liveLower) {
    if (hasSelectionMenu(lines) ||
    ...
    if (hasInterruptHint(liveLower) || liveLower.includes('• working ('))
        return 'working';
    return 'idle';
}
function detectOpenCode(lines, liveLower) {
    if (hasSelectionMenu(lines) ||
        liveLower.includes('[y/n]') ||
        liveLower.includes('(y/n)') ||
    ...
    if (hasInterruptHint(liveLower) || hasSpinnerActivity(lines))
        return 'working';
    return 'idle';
}
function detectCursor(lines, liveLower) {
    if (hasSelectionMenu(lines) ||
...
function detectDroid(lines, liveLower) {
    if (hasSelectionMenu(lines) ||
        liveLower.includes('[y/n]') ||
        liveLower.includes('(y/n)') ||
        liveLower.includes('allow this') ||
...
function detectCopilot(lines, liveLower) {
    if (hasSelectionMenu(lines) ||
        liveLower.includes('[y/n]') ||
        liveLower.includes('(y/n)') ||
        liveLower.includes('allow command') ||
 ...
 */
export function detectState(agent, screen) {
