...
function storeSessionWindows(sessionName, windows) {
...
export function captureAndStoreWindows(sessionName) {
    const live = captureSessionWindowsWithPath(sessionName);
    const worktreeCache = new Map();
    const captured = live.map((w) => {
    ...
    if (captured.length) {
        storeSessionWindows(sessionName, captured.map((w) => ({ index: w.index, name: w.name, worktree: w.worktree })));
    }
    return captured;
}
// Sidebar data path: lowdb only, no tmux. Merges custom labels on top of the
// last-captured window list for the session.
export function getStoredWindows(sessionName) {
