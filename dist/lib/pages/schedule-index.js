import{cssVarsStyle as y}from"../theme.js";import{commandbarCSS as S,commandbarHTML as k,commandbarScript as $}from"../commandbar.js";import{notesDrawerCSS as E,notesDrawerHTML as L,notesDrawerScript as A}from"../notes-drawer.js";import{sharedLayoutCSS as B,sharedHeader as C,sharedSidebar as M,newSessionModalHTML as q,newSessionModalScript as I}from"../shared-layout.js";import{DELAY_INVALID_MESSAGE as D,DELAY_MAX_MESSAGE as R,MAX_SCHEDULE_MS as T,scheduleDelayParseScript as z}from"../schedule-delay.js";function t(s){return s.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function p(s){const n=new Date(s);return n.toLocaleDateString()+" "+n.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}function N(s,n){return s.length?`<div class="task-list">${s.map(e=>`<div class="task">
  <div class="task-row1">
...
`)}</div>`:`<p class="empty">No tasks triggered in the last ${n} day${n===1?"":"s"}.</p>`}function G(s,n,c,e=7,i=!1,u=[]){const d=[...s].sort((r,o)=>r.fireAt-o.fireAt),l=new Map;for(const r of d){const o=l.get(r.sessionName);o?o.push(r):l.set(r.sessionName,[r])}const m=[...l.entries()].map(([r,o])=>{const w=o.map(a=>`<div class="task" data-id="${t(a.id)}">
  <div class="task-row1">
...
`);return`<div class="session-group" data-session="${t(r)}">
  ...
  <div class="task-list">${w}</div>
</div>`}).join(`
`),h=d.length?m:'<p class="empty" id="empty-msg">No scheduled tasks.</p>',f=N(n,e),g=T,b=D,x=R,v=`
  .session-head {
...
${C({commandbarEnabled:i,title:"Scheduled",themeTemplate:c.template})}

<div class="page-wrap">
  <div class="page-layout">
    ${M({activePage:"schedule",refreshHref:"/schedule"})}
...
async function submitReschedule(id, delayStr) {
  ...
  if (!delayMs) {
    ...
    errorEl.textContent = ${JSON.stringify(b)};
    input.focus();
    return;
  }
...
</html>`}export{G as renderScheduleIndex};
