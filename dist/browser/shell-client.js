...
sidebarContent.addEventListener('click', (e) => {
    const el = e.target.closest('.sidebar-git');
    ...
    if (!files.length) {
        ...
        gitPopover.querySelectorAll('.git-popover-item').forEach((el) => {
            ...
            if (!file || file.status === 'D')
                return; // deleted files can't show diff
...
function buildDiffHtml(diff, stagedDiff) {
...
function renderDiffInPopover(filePath, diff, stagedDiff) {
    if (!diff && !stagedDiff) {
        gitPopover.innerHTML = `<div class="git-diff-view"><div class="diff-header">
			<button class="diff-back">← Back</button>
			<span class="diff-filename">${escHtml(filePath)}</span>
...
function renderDiffInMain(filePath, diff, stagedDiff) {
    if (!diff && !stagedDiff) {
