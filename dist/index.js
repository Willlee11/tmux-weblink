...
import { renderTerminal, renderLoginPage, renderNotesIndex, renderNotesPage, renderSettings, renderThemeSettings, renderScheduleIndex, renderAgentsIndex, renderHistoryIndex, renderQuickCommandsPage, renderFilesIndex, renderShell } from "./frontend.js";
...
import { probeWatchedPanes, startBackgroundWatch, getCachedAgentStatuses, requestProbe, } from "./lib/agents-watch.js";
loadDotEnv();
...
function recordActivePane(session) {
    if (!agentsEnabled)
        return;
    try {
...
app.get("/api/agents", requireAuth(), async (c) => {
    if (!agentsEnabled)
        return c.json({ error: "agents page disabled" }, 403);
